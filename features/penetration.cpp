...
ReturnInfo_t penetration::think( vec3_t pos, c_cs_player* target, int specific_hitgroup ){
	...
	float range = min( weapon_info->m_range, ( start - pos ).length( ) );
...
void penetration::scale_damage( c_cs_player* e, c_cs_weapon_data* weapon_info, int hitgroup, float& current_damage ){
	...
	static auto mp_damage_scale_t_body = interfaces::cvar->find_var( _( "mp_damage_scale_t_body" ) );
...
bool penetration::handle_bullet_penetration( c_cs_weapon_data* info, FireBulletData_t& data, bool extracheck, vec3_t point ){
	...
	surfacedata_t* exit_surface_data = interfaces::surface_data->get_surface_data( trace_exit.m_surface.m_surface_props );
	int exit_material = exit_surface_data->game.material;
	float exit_surf_penetration_modifier = exit_surface_data->game.penetration_modifier;

	static auto dmg_reduction_bullets = interfaces::cvar->find_var( "ff_damage_reduction_bullets" )->get_float( );
	static auto dmg_bullet_penetration = interfaces::cvar->find_var( "ff_damage_bullet_penetration" )->get_float( );
	...
	float lost_damage = max( ( ( modifier * thickness ) / 24.f ) + ( ( data.m_current_damage * final_damage_modifier ) + ( max( 3.75f / info->m_penetration, 0.f ) * 3.f * modifier ) ), 0.f );
...
bool penetration::trace_to_exit( c_game_trace* enter_trace, vec3_t start, vec3_t dir, c_game_trace* exit_trace ){
	...
	do{
		...
		if ( !first_contents ) first_contents = interfaces::trace->get_point_contents( end, MASK_SHOT | CONTENTS_GRATE, NULL );
...
void penetration::clip_trace( vec3_t& start, vec3_t& end, c_cs_player* e, unsigned int mask, i_trace_filter* filter, c_game_trace* old_trace ){
	...
	if ( range <= 60.f ) //55.f {
		...
		interfaces::trace->clip_ray_to_entity( ray, mask, e, &trace );
...
bool penetration::can_hit_floating_point( const vec3_t& point, const vec3_t& source ){
	...
	vec3_t angles = math::calculate_angle( data.m_start, point );
	math::ang_vec( angles, data.m_direction );
